...
将本例写入标准接口时，正常时间单元的`scale=28800`秒、`status=1`；将已有占用和维护合并后，`used=(120+30)/480=0.3125`。可用时间为`28800×(1−0.3125)=19800`秒，即330分钟。维护已计入该比例，后续不再扣除30分钟。
...
`time_unit.id`是引用标识，`offset`表达相对顺序，`scale`表达该时间单元的长度，标准单位为秒。实际时间位置需结合起点及时间序列解释；连续编号本身不代表天数或分钟数。

## 比例、数量和时间分别检查
`capacity.used`表示已有占用或不可用比例，范围0到1，25%应为0.25；写成25或120分钟都会改变含义。界面可以显示百分比，但存储、计算与显示必须约定一致。该输入比例与求解结果中的资源利用率分别计算。
