...
工作台用于查看样例输入、配置运行流程，并比较基线与调整后的结果。公开站点可以查看标准样例快照、整理配方和生成 Agent 任务包；实际求解通过本机服务连接已经安装的 DecisioWorks。
...
4. 当前工作台支持 `production_planning` 和 `production_scheduling` 两个标准样例。运行目录需要包含这两个样例及其场景配方。
...
工作台服务本身使用 Python 标准库；运行子进程使用 `--python` 指定的 DecisioWorks 环境。`--runtime` 应指向完整的 DecisioWorks 发行目录，运行组件与依赖版本按该发行版说明准备。
...
能力编排配方（recipe）以 JSON 导出，包含标准动作、输入源和运行参数。四个必需动作按顺序执行，分析与结果记录可选择。当前编辑器采用固定动作顺序；任意脚本节点、DAG 连线和自定义插件执行不在其支持范围内。

Agent 任务包包含任务说明、结构化上下文、验收条件、允许动作清单及配方，不附带原始数据明细或授权文件。导出后可交给所选 Agent 工具使用；工作台本身不会调用外部 AI 服务执行任务。分享前请检查任务说明中是否包含需要保密的业务内容。
...
- 当前适配接口依据 DecisioWorks v1.4.0。升级后，先确认样例数据可读、参数范围与授权状态正确，再运行一次基线与重算，比较结果和错误提示是否符合预期。
